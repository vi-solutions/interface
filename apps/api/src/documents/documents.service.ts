...
export class DocumentsService {
  ...
  async create(dto: CreateDocumentDto, userId: string): Promise<Document> {
    const id = uuid();
    const fileId = this.extractGoogleDriveFileId(dto.googleDriveUrl);
    const { rows } = await this.pool.query(
      `INSERT INTO documents (id, project_id, name, google_drive_url, google_drive_file_id, mime_type, uploaded_by)
